...
void MainWindow::readData()
{
    ...
    QString tmp_data = message;
    QString frame;
    QString outputData;
    int pos = message.indexOf(flag);
    if (pos != -1){
        tmp_data.remove(0,pos+8);
        pos = tmp_data.indexOf(flag);
        //если флаг есть в кадре
        if (pos != -1)
        {
        frame = flag + tmp_data.left(pos);
        }
        else{
        frame = flag + tmp_data;
        }
    ...
    ui->outputBox->setText(ui->outputBox->toPlainText() + outputData);
...
void MainWindow::on_InputBox_returnPressed()
{
    ...
    ui->StatusBox->setText(ui->StatusBox->toPlainText() + "\n");
    sendData(inputText);
    ui->InputBox->clear();
}

void MainWindow::sendData(QString inputText)
{
    QString frame;
    while(inputText.length()>0)
    {
        ...
        frame = getFrame(tmp_data);
        frame = getBitStaffing(frame);
        QString buffer;
        int muxCollision = 0;
        ...
        while(true)
        {
            ...
            {
                ui->StatusBox->setText(ui->StatusBox->toPlainText() + "+");
                ui->StatusBox->moveCursor(QTextCursor::End);
                ui->StatusBox->repaint();
                muxCollision++;
                if (muxCollision<16)
                    Sleep(makeBackoff(muxCollision));
                else
                    break;
            }
            else
...
QString MainWindow::getFrame(QString data)
{
    QString frame;
